...
static struct rel_job_st *rel_job[RELJOB_MAX];
...
static void fetch_stat_unlocked(int id,struct rel_stat_st *stat)
{
...
int rel_statjob(int id,struct rel_stat_st *stat)
{
	...
	pthread_mutex_lock(&mut_rel_job);
	if(id < 0 || id >= RELJOB_MAX || rel_job[id] == NULL || stat == NULL)
	{
		pthread_mutex_unlock(&mut_rel_job);
		return -EINVAL;
	}

	fetch_stat_unlocked(id,stat);	
